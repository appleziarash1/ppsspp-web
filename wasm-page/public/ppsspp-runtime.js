...
// Mounting through WORKERFS aborts the Emscripten module in the threaded build
// (FS.mount throws "Aborted" and leaves the heap unusable, so the MEMFS fallback
// then runs on a dead runtime and the canvas stays black). Keep the path but
// require an explicit opt-in; MEMFS is the working default.
const ENABLE_WORKERFS_FAST_MOUNT = false;
...
function mountGameFileFast(FS, file, safeName, label) {
  if (!ENABLE_WORKERFS_FAST_MOUNT) {
    log("WORKERFS fast mount disabled; using MEMFS for " + (label || safeName) + ".", "info");
    return null;
  }
...
/* ── Virtual gamepad ────────────────────────────────────────────────
   On-screen d-pad/face buttons for touch devices, plus an analog stick that
   is drawn over the canvas but feeds PPSSPP through the mouse events SDL
   already maps to its analog axis.

   Two injection paths, because the browser draws a line between them:
     - buttons  -> synthesized KeyboardEvent (SDL ignores event.isTrusted, so
                   untrusted events still reach the emulator)
     - stick    -> pointer position. Only *trusted* pointer events move the SDL
                   pointer state, so a real finger or a real mouse must drive
                   this one; a synthesized event would look correct on screen
                   yet do nothing to the emulator. */
const VPAD_HIDE_KEY = "ppsspp-vpad-hidden";

// PPSSPP's default keyboard map (Core/KeyMapDefaults.cpp, DEFAULT_MAPPING_KEYBOARD).
// D-pad entries are the literal key names NKCODE_DPAD_*; SDL keeps them verbatim.
const VPAD_KEYMAP = {
  up:       { key: "ArrowUp",    code: "ArrowUp",    keyCode: 38 },
  down:     { key: "ArrowDown",  code: "ArrowDown",  keyCode: 40 },
  left:     { key: "ArrowLeft",  code: "ArrowLeft",  keyCode: 37 },
  right:    { key: "ArrowRight", code: "ArrowRight", keyCode: 39 },
  square:   { key: "a", code: "KeyA", keyCode: 65 },
  triangle: { key: "s", code: "KeyS", keyCode: 83 },
  circle:   { key: "x", code: "KeyX", keyCode: 88 },
  cross:    { key: "z", code: "KeyZ", keyCode: 90 },
  ltrigger: { key: "q", code: "KeyQ", keyCode: 81 },
  rtrigger: { key: "w", code: "KeyW", keyCode: 87 },
  start:    { key: " ", code: "Space", keyCode: 32 },
  select:   { key: "Enter", code: "Enter", keyCode: 13 },
};

const ANALOG_DEADZONE = 0.14;
const ANALOG_STICK_STEP_MS = 16;

let vpadManualOverride = null;   // true = forced on, false = forced off, null = auto
let vpadAutoShown = null;        // last decision, so we only touch the DOM on change
let vpadStickId = null;
let vpadAnalogTimer = 0;
let vpadAnalogSample = { x: 0, y: 0 };
let vpadPollTimer = 0;

function vpadElements() {
  return {
    root: byId("vpad"),
    toggle: byId("vpadToggle"),
    stick: document.querySelector(".vpad-stick"),
    knob: document.querySelector(".vpad-stick-knob"),
  };
}
...
function activeGamepadList() {
  const getPads = navigator.getGamepads?.bind(navigator);
  if (!getPads) return [];
  return Array.from(getPads()).filter(gp => gp && gp.connected !== false);
}

function shouldAutoShowVpad() {
  if (!started) return false;              // stay out of the way until the game runs
  if (activeGamepadList().length > 0) return false;  // a physical pad took over
  return hasTouchInput();
}

function syncVpadVisibility() {
  const { root, toggle } = vpadElements();
  if (!root) return;

  const auto = shouldAutoShowVpad();
  const visible = vpadManualOverride === null ? auto : vpadManualOverride;
  if (visible !== vpadAutoShown) {
    vpadAutoShown = visible;
    root.classList.toggle("vpad-visible", visible);
    root.setAttribute("aria-hidden", visible ? "false" : "true");
  }
  toggle?.classList.toggle("vpad-toggle-active", visible);
  toggle?.setAttribute("aria-pressed", String(visible));
}

function setVpadManual(next) {
  vpadManualOverride = next;
  syncVpadVisibility();
}

// SDL reads the pointer from real (trusted) events; synthesize them anyway so a
// programmatic caller behaves like a finger would.
function sendCanvasMouse(type, clientX, clientY, buttons) {
  canvas?.dispatchEvent(new MouseEvent(type, {
    bubbles: true, cancelable: true,
...
function stopAnalogStick() {
  if (vpadAnalogTimer) { clearInterval(vpadAnalogTimer); vpadAnalogTimer = 0; }
  vpadAnalogSample = { x: 0, y: 0 };
}

// Held direction on the mouse pointer, re-sent so SDL keeps the axis centred
// until the finger moves it into the deadzone and we release for real.
function startAnalogStick(x, y) {
  stopAnalogStick();
  vpadAnalogSample = { x, y };
  vpadAnalogTimer = setInterval(() => {
    sendCanvasMouse("mousemove", vpadAnalogSample.x, vpadAnalogSample.y, 1);
  }, ANALOG_STICK_STEP_MS);
}

function vpadStickPoint(ev) {
  const { stick } = vpadElements();
  if (!stick) return null;
  const rect = stick.getBoundingClientRect();
  const cx = rect.left + rect.width / 2;
  const cy = rect.top + rect.height / 2;
  const radius = Math.max(rect.width, rect.height) / 2;
  const dx = (ev.clientX - cx) / radius;
  const dy = (ev.clientY - cy) / radius;
  const dist = Math.hypot(dx, dy);
  const scale = dist > 1 ? 1 / dist : 1;
  return {
    x: dx * scale,
    y: dy * scale,
    // The canvas sits under the stick, so translate to stage coordinates.
    clientX: cx + dx * scale * radius,
    clientY: cy + dy * scale * radius,
  };
}

function renderStickKnob(vector) {
  const { stick, knob } = vpadElements();
  if (!stick || !knob) return;
  const rect = stick.getBoundingClientRect();
  const travel = Math.max(rect.width, rect.height) / 2;
  const magnitude = Math.hypot(vector.x, vector.y);
  knob.style.transform =
    `translate(${vector.x * travel}px, ${vector.y * travel}px)` +
    (magnitude > ANALOG_DEADZONE ? " scale(1.08)" : "");
}

function moveAnalogStick(vector) {
  // Map the [0..1] radius onto the canvas centre, where SDL's pointer pinning
  // turns absolute motion into the relative delta PPSSPP uses for the axis.
  const canvasRect = canvas?.getBoundingClientRect();
  if (!canvasRect) return;
  const cx = canvasRect.left + canvasRect.width / 2;
  const cy = canvasRect.top + canvasRect.height / 2;
  const reach = Math.min(canvasRect.width, canvasRect.height) * 0.45;
  const clientX = cx + vector.x * reach;
  const clientY = cy + vector.y * reach;
  vpadAnalogSample = { clientX, clientY };
  renderStickKnob(vector);
  if (vpadAnalogTimer) return;          // already held; the timer carries the motion
  sendCanvasMouse("mousemove", clientX, clientY, 1);
}

function releaseAnalogStick() {
  const { knob } = vpadElements();
  stopAnalogStick();
  vpadStickId = null;
  if (knob) knob.style.transform = "";
  // Only release if the stick actually took the pointer.
  if (canvas) sendCanvasMouse("mouseup", 0, 0, 0);
}

function dispatchVpadKey(macro, type) {
  const mapping = VPAD_KEYMAP[macro];
  if (!mapping) return;
...
function bindVpadButton(btn) {
  const macro = btn.dataset.vpadMacro;
  const press = ev => {
    if (btn.dataset.vpadHeld === "1") return;
    btn.dataset.vpadHeld = "1";
    btn.classList.add("vpad-held");
    dispatchVpadKey(macro, "keydown");
    // Keep the gesture ours; the canvas below must not also receive it.
    ev.preventDefault();
    ev.stopPropagation();
  };
  const release = () => {
    if (btn.dataset.vpadHeld !== "1") return;
    btn.dataset.vpadHeld = "0";
    btn.classList.remove("vpad-held");
    dispatchVpadKey(macro, "keyup");
  };
  ...
  // A button that keeps keyboard focus would swallow the arrows the emulator
  // needs and would re-fire on Space/Enter.
  btn.addEventListener("keydown", ev => ev.preventDefault());
}

function bindAnalogStick() {
  const { stick } = vpadElements();
  if (!stick) return;

  const begin = ev => {
    if (vpadStickId !== null) return;
    vpadStickId = ev.pointerId;
    stick.setPointerCapture?.(ev.pointerId);
    const point = vpadStickPoint(ev);
    if (!point) return;
    // Latch the stick onto the canvas centre before streaming motion.
    canvas?.focus?.();
    const canvasRect = canvas?.getBoundingClientRect();
    if (canvasRect) {
      sendCanvasMouse("mousemove", canvasRect.left + canvasRect.width / 2,
                      canvasRect.top + canvasRect.height / 2, 1);
      sendCanvasMouse("mousedown", canvasRect.left + canvasRect.width / 2,
                      canvasRect.top + canvasRect.height / 2, 1);
    }
    startAnalogStick(point.clientX, point.clientY);
    moveAnalogStick({ x: point.x, y: point.y });
    ev.preventDefault();
    ev.stopPropagation();
  };
  ...
  stick.addEventListener("pointermove", ev => {
    if (ev.pointerId !== vpadStickId) return;
    const point = vpadStickPoint(ev);
    if (!point) return;
    moveAnalogStick({ x: point.x, y: point.y });
    ev.preventDefault();
...
function bindVpad() {
  const { toggle } = vpadElements();
  document.querySelectorAll("[data-vpad-macro]").forEach(bindVpadButton);
  bindAnalogStick();

  if (localStorage.getItem(VPAD_HIDE_KEY) === "1") vpadManualOverride = false;
  on(toggle, "click", () => {
    const { root } = vpadElements();
    const nowVisible = !root?.classList.contains("vpad-visible");
    setVpadManual(nowVisible);
    localStorage.setItem(VPAD_HIDE_KEY, nowVisible ? "0" : "1");
  });

  // Hand control back to the physical pad the moment one shows up, and restore
  // the touch pad if it goes away again.
  window.addEventListener("gamepadconnected", () => syncVpadVisibility());
  window.addEventListener("gamepaddisconnected", () => syncVpadVisibility());
  if (navigator.getGamepads) {
    vpadPollTimer = setInterval(syncVpadVisibility, 1500);
  }
  if (window.matchMedia) {
    window.matchMedia("(pointer: coarse)").addEventListener?.("change", syncVpadVisibility);
  }
  syncVpadVisibility();
  log("Virtual gamepad ready (touch=" + hasTouchInput() + ").", "info");
}
